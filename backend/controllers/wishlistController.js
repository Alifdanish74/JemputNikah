...
const uploadToS3 = async (file, filename) => {
    ...
    try {
      ...
      for (let i = 1; i <= 10; i++) {
        ...
        if (!productName && !productUrl && !productImageFile && !existingImage) {
          // Remove product if no data provided
          delete wishlistEntry[wishlistKey];
          continue;
        }
  
        if (!wishlistEntry[wishlistKey]) {
          wishlistEntry[wishlistKey] = { uniqueId: new mongoose.Types.ObjectId().toString() };
        }
  
        const uniqueId = wishlistEntry[wishlistKey].uniqueId;
        let productImage = null;
  
        // Upload new image or retain the existing one
        if (productImageFile) {
          productImage = await uploadToS3(productImageFile, `${orderNumber}/wishlist_${uniqueId}.png`);
        } else if (existingImage) {
          productImage = existingImage;
        }
  
        // Update or create the wishlist product
        wishlistEntry[wishlistKey] = {
          productName: productName || wishlistEntry[wishlistKey]?.productName || "",
          productUrl: productUrl || wishlistEntry[wishlistKey]?.productUrl || "",
          productImage: productImage || wishlistEntry[wishlistKey]?.productImage || null,
          bookingName: wishlistEntry[wishlistKey]?.bookingName || null,
          bookingPhoneNumber: wishlistEntry[wishlistKey]?.bookingPhoneNumber || null,
          bookingStatus: wishlistEntry[wishlistKey]?.bookingStatus || "Available",
          uniqueId,
        };
      }
      ...
      res.status(201).json({ message: "Wishlist uploaded successfully", wishlistEntry });
    } catch (error) {
  ...
  const updateWishlist = async (req, res) => {
    ...
    try {
      ...
      wishlistEntry[wishlistKey].productName = productName || wishlistEntry[wishlistKey].productName;
      wishlistEntry[wishlistKey].productUrl = productUrl || wishlistEntry[wishlistKey].productUrl;
      wishlistEntry[wishlistKey].bookingStatus = bookingStatus || wishlistEntry[wishlistKey].bookingStatus;
  
      if (productImage) {
        const imageUrl = await uploadToS3(productImage, `wishlist${productIndex}_${orderNumber}.png`);
        wishlistEntry[wishlistKey].productImage = imageUrl;
      }
  
      await wishlistEntry.save();
      res.status(200).json({ message: "Wishlist item updated successfully", wishlistEntry });
    } catch (error) {
